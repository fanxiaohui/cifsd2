...
int smb2_open(struct cifsd_work *work)
{
	...
	struct cifsd_share *share;
	struct cifsd_inode *ino = NULL, *f_parent_ino;
	...
	if (!oplocks_enable || (req_op_level == SMB2_OPLOCK_LEVEL_LEASE &&
		...
		if (req_op_level == SMB2_OPLOCK_LEVEL_LEASE) {
			...
			rc = find_same_lease_key(sess, ino, lc);
			if (rc)
				goto err_out;
		}
	...
	list_add(&fp->node, &ino->m_fp_list);
...
wait:
	...
	if (oplocks_enable && atomic_read(&fp->f_ino->op_count) > 1)
		smb_break_all_oplock(work, fp);
